...
struct MeshCacheBase {
 protected:
  ...
  MeshCacheBase(const Teuchos::RCP<Teuchos::ParameterList>& plist)
    : MeshCacheBase()
  {
 ...
 public:
  ...
  Teuchos::RCP<const MeshAlgorithms> getAlgorithms() const {
    return algorithms_;
  }
...
struct MeshCache : public MeshCacheBase {
  ...
  //
  template <MemSpace_kind MEM_OTHER> MeshCache(MeshCache<MEM_OTHER>& other);
  ...
  const Import_type& getBoundaryFaceInternalCellImporter() const { return maps_.getBoundaryFaceInternalCellImporter(); }
  ...
  template <AccessPattern_kind AP = AccessPattern_kind::DEFAULT>
  KOKKOS_INLINE_FUNCTION void
  getEdgeCells(const Entity_ID e, cEntity_ID_View& cells) const;
  ...
  template <AccessPattern_kind AP = AccessPattern_kind::DEFAULT>
  KOKKOS_INLINE_FUNCTION void
  getEdgeFaces(const Entity_ID edgeid, cEntity_ID_View& faces) const;

  // Cells of type 'ptype' connected to a node
  // NOTE: The order of cells is not guaranteed to be the same for
  // corresponding nodes on different processors
  template <AccessPattern_kind AP = AccessPattern_kind::DEFAULT>
  KOKKOS_INLINE_FUNCTION cEntity_ID_View getNodeCells(const Entity_ID n,
                                                      const Parallel_kind ptype = Parallel_kind::OWNED) const;
  ...
  template <AccessPattern_kind AP = AccessPattern_kind::DEFAULT>
  KOKKOS_INLINE_FUNCTION void
  getNodeFaces(const Entity_ID n, cEntity_ID_View& faces) const;
